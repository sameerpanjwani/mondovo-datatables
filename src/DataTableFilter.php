...
class DataTableFilter extends DataTableAdapter implements DataTableFilterInterface
{
	 ...
	 * @return bool
	 */
	private function processCustomQuery($search_value, $operator, $column_index, &$query, $all_values = '', $filter_type = '')
	{
		...
		} else
		{
			...
			if($filter_type == "json"){
				$query->whereJsonContains($this->getColumnName($column_index), $search_value);
			}else {
				$query->orWhere($this->getColumnName($column_index), $operator, $search_value);
			}
		}
	...
	private function prepareQuery($search_results, $operator, $column_index, &$query)
	{
		...
		foreach ($search_results as $search_concat) {
			...
			$filter_type = isset($search_concat['filter_type']) ? $search_concat['filter_type'] : '';

			$this->concat_condition_in_filter = $this->prepareConcatCondition($column_index, $concat_columns);

			$this->processCustomQuery($search_value, $operator, $column_index, $query, $all_values, $filter_type);
		}
